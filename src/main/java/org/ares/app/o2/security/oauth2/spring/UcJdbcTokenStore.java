package org.ares.app.o2.security.oauth2.spring;
...
public class UcJdbcTokenStore implements TokenStore {

	public UcJdbcTokenStore(DataSource dataSource) {
        ...
        try {
            accessToken = (OAuth2AccessToken)this.jdbcTemplate.queryForObject(this.selectAccessTokenFromAuthenticationSql, new RowMapper<OAuth2AccessToken>() {
                public OAuth2AccessToken mapRow(ResultSet rs, int rowNum) throws SQLException {
                    return UcJdbcTokenStore.this.deserializeAccessToken(rs.getBytes(2));
                }
            }, new Object[]{key});
        ...
        try {
            authentication = (OAuth2Authentication)this.jdbcTemplate.queryForObject(this.selectAccessTokenAuthenticationSql, new RowMapper<OAuth2Authentication>() {
                public OAuth2Authentication mapRow(ResultSet rs, int rowNum) throws SQLException {
                    return UcJdbcTokenStore.this.deserializeAuthentication(rs.getBytes(2));
                }
...
/*        try {
            refreshToken = (OAuth2RefreshToken)this.jdbcTemplate.queryForObject(this.selectRefreshTokenSql, new RowMapper<OAuth2RefreshToken>() {
                public OAuth2RefreshToken mapRow(ResultSet rs, int rowNum) throws SQLException {
                    return UcJdbcTokenStore.this.deserializeRefreshToken(rs.getBytes(2));
                }
            }, new Object[]{this.extractTokenKey(token)});
        } catch (EmptyResultDataAccessException var4) {
            if (LOG.isInfoEnabled()) {
    ...
    public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
        ...
        try {
            accessTokens = this.jdbcTemplate.query(this.selectAccessTokensFromClientIdSql, new UcJdbcTokenStore.SafeAccessTokenRowMapper(), new Object[]{clientId});
        } catch (EmptyResultDataAccessException var4) {
            if (log.isInfoEnabled()) {
    ...
    public Collection<OAuth2AccessToken> findTokensByUserName(String userName) {
        ...
        try {
            //accessTokens = this.jdbcTemplate.query(this.selectAccessTokensFromUserNameSql, new UcJdbcTokenStore.SafeAccessTokenRowMapper(), new Object[]{userName});
        } catch (EmptyResultDataAccessException var4) {
            if (log.isInfoEnabled()) {
    ...
    public Collection<OAuth2AccessToken> findTokensByClientIdAndUserName(String clientId, String userName) {
        ...
        try {
//            accessTokens = this.jdbcTemplate.query(this.selectAccessTokensFromUserNameAndClientIdSql, new UcJdbcTokenStore.SafeAccessTokenRowMapper(), new Object[]{userName, clientId});
        } catch (EmptyResultDataAccessException var5) {
            if (log.isInfoEnabled()) {
    ...
    private final class SafeAccessTokenRowMapper implements RowMapper<OAuth2AccessToken> {
        ...
        public OAuth2AccessToken mapRow(ResultSet rs, int rowNum) throws SQLException {
            try {
                ...
                UcJdbcTokenStore.this.jdbcTemplate.update(UcJdbcTokenStore.this.deleteAccessTokenSql, new Object[]{token});
                return null;
            }
        }
    }
	...
	String updateAccessTokenSql = "update authentication a set a.token_id = ?,a.token = ?,a.token_byte = ?, a.authentication_id = ?, a.authentication = ?, a.refresh_token = ? where id = ?";
	String selectExistClientFromClientNameSql = "select id from authentication where client_name = ? limit 1";
	String selectAccessTokenSql = "select token_id, token_byte from authentication where token_id = ?";
	String selectAccessTokenAuthenticationSql = "select token_id, authentication from authentication where token_id = ?";
	String selectAccessTokenFromAuthenticationSql = "select token_id, token_byte from authentication where authentication_id = ?";
	// 下面SQL的没啥用
	String selectAccessTokensFromClientIdSql = "select token_id, token from authentication where client_id = ?";
	String deleteAccessTokenSql = "delete from authentication where token_id = ?";
